...
ModbusMessage readDiscreteInputs(ModbusMessage request)
{
  ...
  if ((addr + count) > STATUSSIZE)
  { // Apenas 4 entradas digitais disponíveis
    response.setError(request.getServerID(), request.getFunctionCode(), ILLEGAL_DATA_ADDRESS);
  }
  else
  {
    ...
    for (uint8_t byteIndex = 0; byteIndex < byteCount; byteIndex++)
    {
      ...
      for (uint8_t bit = 0; bit < 8; bit++)
      {
        ...
        if (inputIndex < (addr + count))
        {
          ...
          {
          case 0:
            pinState = digitalRead(def_pin_RTN1);
            break;
          case 1:
            pinState = digitalRead(def_pin_RTN2);
            break;
          case 2:
            pinState = digitalRead(def_pin_PUSH1);
            break;
          case 3:
            pinState = digitalRead(def_pin_PUSH2);
            break;
          case 4:
            pinState = digitalRead(def_pin_D1);
            break;
          default:
            break;
          }
...
ModbusMessage readHoldingRegisters(ModbusMessage request)
{
  ...
  if ((addr + words) > HRSIZE)
  { // Apenas HRSIZE registradores disponíveis
    response.setError(request.getServerID(), request.getFunctionCode(), ILLEGAL_DATA_ADDRESS);
  }
  else
  {
...
ModbusMessage readInputRegisters(ModbusMessage request)
{
  ...
  if ((addr + words) > IRSIZE)
  { // Temos 5 entradas analógicas
    response.setError(request.getServerID(), request.getFunctionCode(), ILLEGAL_DATA_ADDRESS);
  }
  else
  {
...
ModbusMessage writeSingleCoil(ModbusMessage request)
{
  ...
  {
  case 0:
    digitalWrite(def_pin_D2, state ? HIGH : LOW);
    break;
  case 1:
    digitalWrite(def_pin_D3, state ? HIGH : LOW);
    break;
  case 2:
    digitalWrite(def_pin_D4, state ? HIGH : LOW);
    break;
  default:
    digitalWrite(def_pin_RELE, state ? HIGH : LOW);
    break;
  }
...
ModbusMessage writeMultipleRegisters(ModbusMessage request)
{
  ...
  for (uint16_t i = 0; i < quantity; i++)
  {
    ...
    case 0:
      ...
      break;
    case 1:
      analogWrite(def_pin_W4a20_1, out);
      break;
    case 2:
      analogWrite(def_pin_PWM, out);
      break;
    }
  }
...
void managerInputFunc(void) {
    const uint16_t vlPOT1 = ads.analogRead(3);
    const uint16_t vlPOT2 = ads.analogRead(2);
